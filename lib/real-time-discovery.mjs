...
const HELIUS_WEBSOCKET_URL = HELIUS_API_KEY 
  ? `wss://mainnet.helius-rpc.com?api-key=${HELIUS_API_KEY}`
  : (process.env.HELIUS_WEBSOCKET_URL || 'wss://mainnet.helius-rpc.com');

console.log(`[GEM-HUNTER] WebSocket URL configured: wss://mainnet.helius-rpc.com?api-key=${HELIUS_API_KEY ? '***' : 'MISSING'}`);
...
const connections = {
  raydium: { ws: null, isReconnecting: false },
  orca: { ws: null, isReconnecting: false },
  pumpfun: { ws: null, isReconnecting: false }
};
...
export async function startTokenDiscovery(onTokenDiscovered) {
  ...
  // Keep the process alive - WebSocket connections will handle events in the background
  return new Promise(() => {
    // This Promise never resolves, keeping the service running indefinitely
    // The WebSocket event listeners will continue to process token discoveries
  });
}
