...
export async function trackSmartMoneyActivity() {
  try {
    ...
    // Get all active smart wallets using direct SQL
    const walletsResult = await query(
      'SELECT * FROM smart_wallets WHERE is_active = true ORDER BY success_rate DESC'
    );

    const smartWallets = walletsResult.rows;
    ...
    for (const wallet of smartWallets) {
      ...
      for (const purchase of purchases) {
        // Record signal in database using direct SQL
        try {
          await query(
            `INSERT INTO smart_money_signals (token_address, wallet_address, signal_type, amount_usd, transaction_hash, detected_at)
             VALUES ($1, $2, $3, $4, $5, $6)`,
            [purchase.tokenAddress, wallet.address, 'BUY', purchase.amountUsd, purchase.txHash, purchase.timestamp]
          );
          totalSignals++;
          ...
          console.error('[WHALE-TRACKER] Error inserting signal:', insertError.message);
        }
      }

      // Update last_checked_at using direct SQL
      await query(
        'UPDATE smart_wallets SET last_checked_at = $1 WHERE address = $2',
