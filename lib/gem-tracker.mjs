...
export async function saveGemDiscovery(gemData) {
  ...
  try {
    ...
    const discoveryData = {
      ...
      // NEW: Potential score fields for early gem detection
      potential_score: gemData.scoreBreakdown?.potential || 0,
      volume_plus_holders_score: gemData.scoreBreakdown?.volumePlusHolders || 0,
      is_early_token: gemData.scoreBreakdown?.isEarly || false,
...
export async function getDiscoveredGems(filters = {}) {
  ...
  try {
    ...
    // Filter out "Unknown" tokens from summaries
    query = query.neq('name', 'Unknown');
