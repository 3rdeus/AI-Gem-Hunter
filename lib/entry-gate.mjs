...
import { analyzeHoneypotRisk, analyzeTokenSecurity } from './goplus.js';
...
export async function evaluateEntry(tokenAddress, tokenCreationTime = null) {
  ...
  try {
    ...
    // ═══════════════════════════════════════════════════════════
    // CHECK 5 & 6: Contract Security (mint/freeze authorities)
    // ═══════════════════════════════════════════════════════════
    const securityData = await analyzeTokenSecurity(tokenAddress);
    if (securityData) {
      // Mint authority
      const mintRevoked = securityData.mintable === '0' || securityData.mintable === false;
      ...
      // Freeze authority
      const freezeRevoked = securityData.freezeable === '0' || securityData.freezeable === false;
      ...
      const top10 = parseFloat(securityData.holder_count) || 0; // Would need actual top10 %
      // Note: GoPlus might not have this directly, may need separate call
      ...
      console.log(`[ENTRY-GATE] Contract Security: check failed - ❌`);
    }
    
    // ═══════════════════════════════════════════════════════════
    // CHECK 7 & 8: Honeypot & Sell Tax
    // ═══════════════════════════════════════════════════════════
    const honeypotData = await analyzeHoneypotRisk(tokenAddress);
    if (honeypotData) {
      checks.honeypot.value = honeypotData.is_honeypot ? 'HONEYPOT' : 'sellable';
      checks.honeypot.passed = !honeypotData.is_honeypot;
      ...
      const sellTax = parseFloat(honeypotData.sell_tax || 0) * 100;
      ...
      failReasons.push('Could not verify honeypot status');
      console.log(`[ENTRY-GATE] Honeypot: check failed - ❌`);
    }
