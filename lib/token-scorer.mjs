  ...
  try {
    ...
    if (!isValidTokenData(data.data)) {
      ...
      if (retryCount < maxRetries) {
        const delay = 30000 * Math.pow(2, retryCount); // 30s, 60s, 120s
        console.log(`[SCORER] 🔄 Retry ${retryCount + 1}/${maxRetries} in ${delay/1000}s...`);
        
        await new Promise(resolve => setTimeout(resolve, delay));
        return fetchTokenData(tokenAddress, retryCount + 1, maxRetries);
      } else {
        console.error(`[SCORER] ❌ Max retries reached, returning zero data`);
        return data.data; // Return zeros after max retries
      }
    }
